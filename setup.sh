...
# fi
# symlink zsh
[ ! -L ${HOME}/.zshrc ] && ln -s ${PWD}/zsh/.zshrc ${HOME}/.zshrc
echo "ZSH setup done"
...
[ ! -L $nvimpath ] && ln -s ${PWD}/nvim/lua_config $nvimpath

echo "Neovim setup done"

# symlink tmux
[ ! -L ${HOME}/.tmux.conf ] && ln -s ${PWD}/tmux/.tmux.conf ${HOME}/.tmux.conf
[ ! -L ${HOME}/.tmux.conf.local ] && ln -s ${PWD}/tmux/.tmux.conf.local ${HOME}/.tmux.conf.local
